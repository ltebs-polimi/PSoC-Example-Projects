...
#warning // Replace I2C_Master with the name of the I2C peripheral in your design
...
#include I2C_Peripheral_Name_Header_File
                                            ...
                                            uint8_t* data)
    {
        ...
        uint8_t error = I2C_Peripheral_Name(MasterSendStart)(device_address,I2C_Peripheral_Name(WRITE_XFER_MODE));
        if (error == I2C_Peripheral_Name(MSTR_NO_ERROR))
        {
            ...
            if (error == I2C_Peripheral_Name(MSTR_NO_ERROR))
            {
                ...
                error = I2C_Peripheral_Name(MasterSendRestart)(device_address, I2C_Peripheral_Name(READ_XFER_MODE));
                if (error == I2C_Peripheral_Name(MSTR_NO_ERROR))
                {
                    // Read data without acknowledgement
                    *data = I2C_Peripheral_Name(MasterReadByte)(I2C_Peripheral_Name(NAK_DATA));
                ...
                if (error == I2C_Peripheral_Name(MSTR_NO_ERROR))
                {
                    ...
                    while(counter>1)
                    {
                        data[register_count-counter] =
                            I2C_Peripheral_Name(MasterReadByte)(I2C_Peripheral_Name(ACK_DATA));
                        counter--;
                    }
                    // Read last data without acknowledgement
                    data[register_count-1]
                        = I2C_Peripheral_Name(MasterReadByte)(I2C_Peripheral_Name(NAK_DATA));
                    // Send stop condition and return no error
                    I2C_Peripheral_Name(MasterSendStop)();
                    return NO_ERROR;
                }
